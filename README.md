...
mvn -v
```
Это возможно работает только для текущего сеанса. При желании можно поставить maven новой версии навсегда. Для этого найдем путь, где установлен maven с репозитория linux; из вывода команды нам нужен Maven Home путь:
```bash
mvn -v
```
